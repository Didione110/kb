...
# ---------------- 回复构建 ----------------
# 问候 / 询问能力 → 返回知识库简介
GREETINGS = re.compile(r"^(你好|您好|hi|hello|嗨|哈喽|在吗|在么|早上好|下午好|晚上好|你好呀|哈喽|谢谢|感谢|好的|ok|收到)[!！。.~～\s]*$", re.I)
# 询问"你能解答哪些问题 / 你会什么 / 你能做什么 / 介绍一下你"
ASK_ABILITY = re.compile(
    r"(你能|可以|会).{0,6}(解答|回答|处理|解决|帮|做什么|干什么|哪些|什么问题|什么)",
    re.I)

# 闲聊 / 与知识库无关 → 礼貌拒绝
CHITCHAT = re.compile(
    r"(吃饭|睡觉|天气|心情|开心|难过|工资|加班|老板|同事|八卦|笑话|故事|电影|音乐|"
    r"游戏|小说|新闻|股票|彩票|结婚|离婚|恋爱|对象|男女朋友|年龄|多大了|"
    r"你是谁|你叫什么|你多高|你多重|你住在哪|你爸妈|你有男朋友|你有女朋友)",
    re.I)
# 敏感 / 不合适内容 → 礼貌拒绝
SENSITIVE = re.compile(
    r"(色情|黄片|裸照|裸体|a片|av|性交|做爱|赌博|毒品|枪支|炸弹|杀人|自杀|诈骗|黑客|入侵|破解|"
    r"翻墙|政治|法轮|邪教|攻击|骂人|脏话|傻逼|操你|去死|诅咒|滚蛋)",
    re.I)
...
def build_reply(question, results):
    ...
    q = question.strip()
    # 1) 问候 / 询问能力 → 知识库简介
    if GREETINGS.match(q) or ASK_ABILITY.search(q):
        return HELP_MSG
    # 2) 闲聊 / 敏感内容 → 礼貌拒绝（不查知识库）
    if CHITCHAT.search(q) or SENSITIVE.search(q):
        return DECLINE_MSG
    # 3) 检索无结果 → 未命中提示
    if not results:
        return ("## 🤖 小福 · 知识库未命中\n\n"
